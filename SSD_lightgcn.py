...
class DeNoiseTrainer(Trainer):
    ...
    def __init__(self, config, model):
        ...
        self.start_denoise = config['start_denoise']
    def fit(self, train_data, valid_data=None, verbose=True, saved=True, show_progress=False, callback_fn=None):
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            # train
            if epoch_idx>self.start_denoise:
                drop_num ,limit = self.model.graph_denoise(epoch_idx-self.start_denoise)
                print("[Structure denoising finished with {} edge dropped, drop theta is {}.]".format(drop_num,limit))
...
class LightSSD(GeneralRecommender):
    input_type = InputType.PAIRWISE

    def __init__(self, config, dataset):
        super(LightSSD, self).__init__(config, dataset)
        ...
        self.rate_min = config['rate_min']
        self.rate_max = config['rate_max']
        self.kind = config['kind']
        self.theta = self.min_epoch ** (1.0 / (self.rate_max - self.rate_min))
    ...
    def create_adjust_matrix(self, re_weight):
        user = self._user
        item = self._item
        matrix = sp.csr_matrix((re_weight, (user, item + self.n_users)),
    ...
    def graph_denoise(self,epoch_idx):
        ...
        neighbor_users2, neighbor_items2 = torch.split(self.restore_list_e[3], [self.n_users, self.n_items])
        ...
        # confidence of observed interactions as weight for denoising
        ...
        if self.kind==0:
            ...
            limit = max(self.rate_max-np.log(epoch_idx+1) / np.log(self.theta), self.rate_min)
        weight = weight * (weight > limit)
        self.norm_adj_matrix = self.csr2tensor(self.create_adjust_matrix(weight))

        return (weight <= limit).sum() ,limit

    def get_batch_weight(self,interaction, embedding_list):
        interact_user = interaction[self.USER_ID]
        interact_item = interaction[self.ITEM_ID]
        neighbor_users1, neighbor_items1 = torch.split(embedding_list[1], [self.n_users, self.n_items])
        neighbor_users2, neighbor_items2 = torch.split(embedding_list[0], [self.n_users, self.n_items])
        train_user_embs1 = neighbor_users1[interact_user]
        train_item_embs2 = neighbor_items2[interact_item]
        train_user_embs1 = F.normalize(train_user_embs1)
        train_item_embs2 = F.normalize(train_item_embs2)
        train_user_embs2 = neighbor_users2[interact_user]
        train_item_embs1 = neighbor_items1[interact_item]
        train_user_embs2 = F.normalize(train_user_embs2)
        train_item_embs1 = F.normalize(train_item_embs1)
    ...
    def calculate_loss(self, interaction):
        ...
        user_all_embeddings, item_all_embeddings, embedding_list = self.forward()
        weight = self.get_batch_weight(interaction, embedding_list)
        ...
        mf_loss = -torch.log(1e-10 + torch.sigmoid((pos_scores - neg_scores)*weight)).mean()
    ...
    def predict(self, interaction):
        ...
        user_all_embeddings, item_all_embeddings, self.restore_list_e = self.forward()
        u_embeddings = user_all_embeddings[user]
...
def run_single_model(args):
    ...
    model = LightSSD(config, train_data.dataset).to(config['device'])
    logger.info(model)

    # trainer loading and initialization
    trainer = DeNoiseTrainer(config, model)
